...
// Gemini CLI OAuth 凭证（官方公开凭证，与 Gemini CLI 相同）
// 这些是 Google 公开的 OAuth 应用凭证，用于 Gemini CLI
const DEFAULT_OAUTH_CLIENT_ID: &str =
    "681255809395-oo8ft2oprdrnp9e3aqf6av3hmdib135j.apps.googleusercontent.com";
const DEFAULT_OAUTH_CLIENT_SECRET: &str = "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";

// OAuth credentials - 优先使用环境变量，否则使用默认公开凭证
fn get_oauth_client_id() -> Option<String> {
    std::env::var("GEMINI_OAUTH_CLIENT_ID")
        .ok()
        .or_else(|| Some(DEFAULT_OAUTH_CLIENT_ID.to_string()))
}

fn get_oauth_client_secret() -> Option<String> {
    std::env::var("GEMINI_OAUTH_CLIENT_SECRET")
        .ok()
        .or_else(|| Some(DEFAULT_OAUTH_CLIENT_SECRET.to_string()))
}
