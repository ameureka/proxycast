...
BACKUP_DIR="$HOME/Desktop/gemini_backups"
...
show_help() {
    ...
    echo "  list             列出所有备份"
    echo "  restore [文件]   恢复指定备份到 ~/.gemini/"
    echo "  help             显示帮助"
    echo ""
    echo "示例:"
...
backup_creds() {
    ...
    if [ ! -f "$CREDS_FILE" ]; then
        ...
        print_info "请先运行 'gemini auth login' 登录"
        exit 1
    fi
    ...
    if [ -n "$custom_name" ]; then
        ...
        local backup_name="${safe_email}_${timestamp}.json"
    else
        local backup_name="gemini_backup_${timestamp}.json"
    fi
    ...
    echo ""
    print_info "下一步操作："
    echo "  1. 运行 'gemini auth login' 登录其他账号"
...
restore_backup() {
    ...
    # 确认操作
    print_warning "这将覆盖当前的 Gemini 凭证！"
